...
const getProduct = (async () => {
  try {
      const resp = await axios.get('http://localhost:3000/product/get-all?type=' + props.type);
...
<template>
  <div class="shadow-xl swiper" ref="recommendationRef">
  <!-- Additional required wrapper -->
  <div class="swiper-wrapper">
    <!-- Slides -->
    <div v-for="item, index in recommendation" :key="index" class="shadow-xl swiper-slide">
      ...
      <a :href="`product/${item.id}`" class="pro__place-link block">
        <img :src="`http://localhost:3000/images/${item.image}`" alt="Hà Nội" class="m-auto pro__place-img rounded-[5px] !h-[300px]">
        <div class="pro__place-title mt-2">
          <p class="text-header font-bold text-center text-lg p-2">{{ item.name }}</p>
          ...
          <p class="text-justify pro__place-decs-place p-2 text-content text-sm duration-300">{{ item.description }}</p>
        </div>
      </a>
    </div>
  </div>
